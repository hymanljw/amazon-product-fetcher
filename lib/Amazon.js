...
class AmazonScraper {
    ...
    httpRequest({ uri, method, qs, json, body, form }) {
        ...
        const asyncResponseId = (qs ? qs.k : uri).replaceAll(/[\W]+/g, "-");
        return new Promise(async (resolve, reject) => {
            const options = {
                ...
                headers: {
                    ...
                    'x-unblock-async-response-id': asyncResponseId
                },
                strictSSL: false,
                ...(json ? { json: true } : {}),
                gzip: true,
            ...
            console.info(`qs => ${JSON.stringify(qs)}, proxy => ${options.proxy}`)
            let tryTimes = 0
            while (tryTimes < 3) {
                try {
                    tryTimes++
                    ...
                    console.error(`error !!!, tryTimes =====> ${tryTimes} `)
                    await this.sleep(1)
                    if (tryTimes >= 3) {
                        reject(error);
                    }
    ...
    async startScraper() {
        if (this.scrapeType === 'products') {
            var pageNum = Math.ceil(this.number / 15)
            this.asyncPage = this.maxPage ? Math.min(...[pageNum, this.maxPage]) : pageNum;
            if (!this.keyword) {
    ...
    grabReviews(body) {
        ...
        for (let key in scrapingResult) {
            ...
            try {
                scrapingResult[key].profileURL = this.mainHost + search[0].attribs.href;
            } catch (error) {
                continue;
            }
        }
         ...
         */
        for (let key in scrapingResult) {
            const search = $(`#${key} [data-hook="helpful-vote-statement"]`);
            scrapingResult[key].helpful_vote = 0;

            try {
                const strVoteCount = $(search[0]).text().toString().split(' ')[0];
                // console.log(strVoteCount, $(search[0]).text().toString().split(" "));
                if (strVoteCount == 'One') {
                    scrapingResult[key].helpful_vote = 1;
                } else if (strVoteCount !== '') {
                    scrapingResult[key].helpful_vote = parseInt(strVoteCount);
                }
            } catch (error) {
                continue;
            }
    ...
    grabProduct(body, p) {
        ...
        for (let i = 0; i < productList.length; i++) {
            if (this.cli) {
                ...
                spinner.text = `Found ${this.collector.length + productList.length} products: ${this.collector.length} and ${productList.length}`;
            }
        ...
        for (let key in scrapingResult) {
            try {
                ...
                const positionSearch = $(`div[data-index][data-asin=${key}]`)[0];
                const titleSearch = $(`div[data-asin=${key}] [data-cy="title-recipe"] a`).last();
                const titleThumbnailSearch = $(`div[data-asin=${key}] [data-image-source-density="1"]`)[0];
                const sponsored = $(`div[data-asin=${key}] span.puis-sponsored-label-info-icon`)[0];
                ...
                if (titleSearch) {
                    scrapingResult[key].title = titleSearch.text();
                }
                ...
                if (titleThumbnailSearch) {
                    ...
                    let url = titleThumbnailSearch.parent.parent.attribs.href;
                    if (url) {
                        if (!url.startsWith(this.mainHost)) {
                            url = this.mainHost + url;
                        }
                        scrapingResult[key].url = decodeURIComponent(url);

                        let match_position = scrapingResult[key].url.match(/ref=sr_\d+_(\d+)/);
                        if (match_position) {
                            scrapingResult[key].position.global_position = match_position[1];
                        }
                        if (scrapingResult[key].url.match(/\/sspa\//)) {
                ...
                if (positionSearch) {
                    scrapingResult[key].position.position = parseInt(positionSearch.attribs['data-index']) - 2;
                }
                ...
                if (ratingSearch) {
                    ...
                    scrapingResult[key].reviews.total_reviews = parseInt(
                        ratingSearch.parent.parent.parent.next.children[0].children[0].attribs['aria-label'].replace(/[\,|\.]/g, ''),
                    );
